...
Game::Game()
{
    ...
    core::Window::Settings settings{ 1280, 720, "Client" };
...
void Game::update()
{
    ...
    if (input.isKeyPressed(core::Key::F1))
        spdlog::info("F1 Pressed");

    if (input.isKeyPressed(core::Key::W))
        spdlog::info("W Pressed");

    if (input.isKeyDown(core::Key::W))
        spdlog::info("W Held");

    if (input.isKeyReleased(core::Key::W))
        spdlog::info("W Released");

    if (input.isMouseButtonReleased(core::Button::Left))
