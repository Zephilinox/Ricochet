...
TEST_CASE("Signal/ManagedConnection")
{
    ...
    {
        // can connect and disconnect
        core::ManagedConnection mc = signal.connect(lambda);
        CHECK(mc);
...

    {
        ...
        auto mc = std::make_unique<core::ManagedConnection>(signal.connect(lambda));
        CHECK(*mc);
